...
bool Turtle::forward(mp_float_t length) {
  ...
  return goTo(m_x + length * std::cos(m_heading * k_headingScale),
              m_y + length * std::sin(m_heading * k_headingScale));
}

void Turtle::left(mp_float_t angle) { setHeading(m_heading + angle); }
...
void Turtle::circle(mp_int_t radius, mp_float_t angle) {
  ...
  if (length > 1) {
    for (int i = 1; i < length; i++) {
      ...
      setHeadingPrivate(oldHeading +
                        std::copysign(angle * progress, angle * radius));
    }
    forward(1);
...
bool Turtle::goTo(mp_float_t x, mp_float_t y) {
  ...
  enum PrincipalDirection { None = 0, X = 1, Y = 2 };

  PrincipalDirection principalDirection =
      xLength > yLength ? PrincipalDirection::X
                        : (xLength == yLength ? PrincipalDirection::None
...
void Turtle::write(const char* string) {
  ...
  KDCoordinate headOffsetY = k_invertedYAxisCoefficient * headOffsetLength *
                             std::sin(m_heading * k_headingScale);
...
void Turtle::viewDidDisappear() { m_drawn = false; }
...
void Turtle::setHeadingPrivate(mp_float_t angle) {
  ...
  mp_float_t angleBetween0And360 =
      angle - ((angle >= 0 && angle < angleLimit)
                   ? 0
                   : std::floor(angle / angleLimit) * angleLimit);
...
KDPoint Turtle::position(mp_float_t x, mp_float_t y) const {
  return KDPoint(std::floor(x + k_xOffset),
                 std::floor(k_invertedYAxisCoefficient * y + k_yOffset));
}
...
bool Turtle::hasDotMask() {
  ...
  for (int j = 0; j < m_penSize; j++) {
    for (int i = 0; i < m_penSize; i++) {
      mp_float_t distance =
          sqrt((j - middle) * (j - middle) + (i - middle) * (i - middle)) /
          (middle + 1);
...
bool Turtle::draw(bool force) {
  ...
  if ((m_speed > 0 || force) && m_visible && !m_drawn &&
    ...
    // Draw the body
    KDRect drawingRect = KDRect(position().translatedBy(KDPoint(
                                    -k_iconBodySize / 2, -k_iconBodySize / 2)),
                                k_iconBodySize, k_iconBodySize);
    ...
    drawingRect = KDRect(
        position()
            .translatedBy(headOffset)
            .translatedBy(KDPoint(-k_iconHeadSize / 2, -k_iconHeadSize / 2)),
        k_iconHeadSize, k_iconHeadSize);
    ctx->fillRect(drawingRect, m_color);
  ...
  if (m_mileage > k_mileageLimit) {
    if (micropython_port_interruptible_msleep(
            1 + (m_speed == 0 ? 0 : 3 * (k_maxSpeed - m_speed)))) {
      return true;
    }
...
bool Turtle::dot(mp_float_t x, mp_float_t y) {
  ...
  uint16_t additionalMileage =
      sqrt((x - m_x) * (x - m_x) + (y - m_y) * (y - m_y)) * 1000;
  m_mileage = ((m_mileage > k_mileageLimit) &&
               ((m_mileage + additionalMileage) < k_mileageLimit))
                  ? k_mileageLimit + 1
                  : m_mileage + additionalMileage;
...
void Turtle::drawPaw(PawType type, PawPosition pos) {
  ...
  float pawX = pawOffset * std::cos(m_heading * k_headingScale + currentAngle) +
               crawlDelta * std::cos(m_heading * k_headingScale);
  float pawY =
      k_invertedYAxisCoefficient *
      (pawOffset * std::sin(m_heading * k_headingScale + currentAngle) +
       crawlDelta * std::sin(m_heading * k_headingScale));
